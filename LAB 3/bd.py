...
def bidirectional_dfs(graph, start, target):
    ...
    while frontier_start and frontier_target:
        ...
        if frontier_target:
            current_target = frontier_target.pop()
...
path_bi = bidirectional_dfs(city_map, start, target)
print(f"Bi-directional DFS Path: {path_bi}")

# Run Standard DFS
path_std = dfs(city_map, start, target)
print(f"Standard DFS Path: {path_std}")

# Visualize the path found by Bi-directional DFS
print("Path Visualized (Bi-directional DFS):")
